...
Leap.loop({ hand: function(hand) {
  ...
  if (gameState.get('state') == 'setup') {
    ...
    if (!grabbedShip && isGrabbing) {
      var intersection = getIntersectingShipAndOffset(cursorPosition);
      grabbedShip = intersection.ship;
      grabbedOffset = intersection.offset;
    }
...
var processSpeech = function(transcript) {
  ...
  if (gameState.get('state') == 'setup') {
    ...
    var shipsDeployed = true;
    if (userSaid(transcript, ["start"])) {
      for(let i =0; i <playerBoard.get('ships').length; i++) {
        var ship = playerBoard.get('ships').models[i];
        if (! ship.get('isDeployed')){
          generateSpeech("Please place your ships on the board!");
          shipsDeployed = false;
          break;
        }
      }
...
var registerPlayerShot = function() {
  ...
  if (!selectedTile) {
    generateSpeech("Your shot was off-board!");
  }
  ...
  // If aiming at a tile, register the player's shot
    ...
    if (result.isGameOver) {
      ...
      var audio = new Audio("./audio/yeah-baby.mp3");
      audio.play();
    ...
    // Sunk ship
      ...
      generateSpeech("You sunk my ".concat(shipName));
    }
    // Hit or miss
    else {
...
var generateCpuShot = function() {
  ...
  generateSpeech("fire ".concat(rowName, " ", colName));
  blinkTile({row: tile.row, col: tile.col});
};
...
var registerCpuShot = function(playerResponse) {
  ...
  if (playerResponse=="game over") {
    if (result.isGameOver)
      generateSpeech("Good game!");
    else
      generateSpeech("The game shouldn't be over");
  }
  else if (playerResponse=="sunk") {
    if (result.isGameOver)
      generateSpeech("Actually the game is over. Good game!");
    else if (result.sunkShip)
      generateSpeech("Let's go!");
    else
      generateSpeech("You sure I sunk your ship?");
  }
  else if (playerResponse=="hit") {
    if (result.isGameOver)
      generateSpeech("Actually the game is over. Good game!");
    else if (result.sunkShip)
      generateSpeech("Actually I think the ship was sunk!");
    else if (result.shot.get('isHit'))
      generateSpeech("Awesome!");
    else
      generateSpeech("No lying! You sure I hit your ship?")
  }
  else if (playerResponse=="miss") {
    if (result.isGameOver)
      generateSpeech("I'm pretty sure I won!");
    else if (result.sunkShip)
      generateSpeech("You're cheating your ship was sunk!");
    else if (result.shot.get('isHit'))
      generateSpeech("No lying! You sure it missed?");
    else
      generateSpeech("Shucks")
  }

  // Game over
  if (result.isGameOver) {
    gameState.endGame("cpu");
    var audio = new Audio("./audio/game-over.m4a");
    audio.play();
    return;
  }
  // Sunk ship
